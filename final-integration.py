...
def getFaceDirection(shape, size):
    image_points = np.array([
                                ...
                                shape[37],    # Right eye right corner
...
def writeEyes(a, b, img):
    ...
    x1 = a[0][0]
    x2 = a[3][0]
    cv2.imwrite('Assets/left-eye.jpg', img[y1:y2, x1:x2])
    y1 = max(b[1][1], b[2][1])
    y2 = min(b[4][1], b[5][1])
    x1 = b[0][0]
    x2 = b[3][0]
    cv2.imwrite('Assets/right-eye.jpg', img[y1:y2, x1:x2])
# open_avg = train.getAvg()
# close_avg = train.getAvg()

def play_with_duration(file_path, duration):
    ...
    player.stop() 
alert = vlc.MediaPlayer('Assets/focus.mp3')
#play_with_duration('Assets/focus.mp3',2)
...
while(True):
    ...
    if(len(rects)):
        ...
        if(avgEAR<close_thresh):
            flag+=1
            eyeContourColor = (0,100,255)
            print(flag)
            if(yawn_countdown and flag>=frame_thresh_3):
                eyeContourColor = (120, 60, 255)
        ...
        if(map_counter>=3):
            map_flag=1
            map_counter=0
            play_with_duration('Assets/take_a_break.mp3',2)
